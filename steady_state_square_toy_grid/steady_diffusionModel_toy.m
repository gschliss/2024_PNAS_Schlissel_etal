classdef steady_diffusionModel_toy
    ...
    methods
        ...
        % generate a grid of polygons ; hasn't been tested outside of squares and hexagons
        function [x_vertices , y_vertices] = generate_polygon_field(obj , square_radius , gridSize , n_vertices)
            ...
            % Loop through rows and columns to create polygons
            for i = 1:gridSize
                y_offset = (i-1) * side_length;
                for j = 1:gridSize
                    x_offset = (j-1) * side_length;
		...
		% run the simulations and report the msds per time step, the number fluxxed per time step, and the final_dist_from_sender for each molecule
        ...
        function [new_x , new_y , new_cell] = generate_new_coordinates(obj ,old_x , old_y , pop , old_cell , grid_x , grid_y  , rates , constrained)
			...
			% handle 2 cases for constrained or free
            switch pop
                case 1 %% pop 1 is either constrained or free
                    D = rates(1);
                    if constrained
                        if old_cell == 0
                            old_cell = obj.update_cell(old_x , old_y , grid_x , grid_y); % for particles that are *newly* confined, give them a cell
                        end
                        tau = 0.006; % 6 ms
                        L = sqrt(2*D*tau);
                        delta_x = normrnd(0,L);
		...
		% update a molecule's population based on the transition matrix
		...
		% initialize a molecule's population
		...
		% convert the tx_rate and fraction_fast into a transition matrix
		...
		% deterine which cell is "home" to a molecule, based on the molecule's postion
        function new_cell = update_cell(obj ,x , y , grid_x , grid_y)
            
            new_cell = 0;
            for i = 1:size(grid_x,1)
                if obj.is_point_inside_polygon(x , y , grid_x(i,:) , grid_y(i,:))
                    new_cell = i;
                    break;
                end
            end
		...
		%% deprecated way to build a square cell field
